...
class NRP_NSGA2:
    ...
    def evaluate_fitness(self):
        ...
        for i in range(self.pop_size):
            ...
            if self.bound <= cost:
                ...
                elif self.fittest['profit'] < profit:
                    self.fittest = self.pop[i]

                elif self.fittest['profit'] == profit and self.fittest['cost'] < cost:
                    self.fittest = self.pop[i]

    # Fast Non-dominated Sorting : Form the non-dominated set(F_i)
    def fast_NS(self):
        F = []
        F_1 = []
        for p in self.pop:
            ...
            p['Domt_num'] = 0
            for q in self.pop:
                if q['profit'] < p['profit'] and q['cost'] < p['cost']:   # if p dominates q
                    p['Domt_set'].append(q)
                elif q['profit'] > p['profit'] and q['cost'] > p['cost']:   # if q dominates p
                    p['Domt_num'] += 1
    ...
    def crowding_distance(self, F_i):
        ...
        # calculate the crowding distance for each objective function.
        for obj in ['profit', 'cost']:
            F_i = sorted(F_i, key=lambda elm: elm[obj])
            F_i[0]['dist'], F_i[len(F_i)-1]['dist'] = 100000000, 100000000  # the min. and max. value.
    ...
    def run_NSGA2(self):
        ...
        while it < self.iteration:
            if it % 10 == 0:
                print(it, self.fittest['profit'], self.fittest['cost'] - self.bound)
            F = self.fast_NS()
            self.pop = []
            i = 0
            ...
            cut = int(len(F[i]) - (self.pop_size/2) + len(self.pop))
            if self.fittest['rank']-1 < i:
                self.pop = self.pop + F[i][cut:]

            elif self.fittest['rank']-1 == i:
                j = 0
                while j < cut + 1:
                    if np.array_equal(self.fittest['chrm'], F[i][j]['chrm']):
                        self.pop = self.pop + F[i][cut+1:] + [self.fittest]
                        break
                    j += 1
                if j == cut + 1:
                    self.pop = self.pop + F[i][cut:]
                ...
                self.pop = self.pop + [self.fittest] + F[i][cut+1:]
